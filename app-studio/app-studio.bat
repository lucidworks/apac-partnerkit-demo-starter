...
set START_TIMEOUT=240
set START_MESSAGE_TEXT="start.txt"
set DOCKER_IMAGE=app-studio-enterprise
set DOCKER_COMMAND=""
...
IF NOT "%SCRIPT_CMD%" == "stop" IF NOT "%SCRIPT_CMD%" == "start" IF NOT "%SCRIPT_CMD%" == "package" IF NOT "%SCRIPT_CMD%" == "dist" IF NOT "%SCRIPT_CMD%" == "docker" (
  CALL :print_usage "" "%SCRIPT_CMD% is not a valid command!"
  EXIT /B 1
)

IF "%SCRIPT_CMD%" == "docker" (
  @REM verify the docker command is supported
  IF NOT "%2" == "package" IF NOT "%2" == "start" IF NOT "%2" == "-help" IF NOT "%2" == "-usage" (
    CALL :print_usage "" "%2 is not a valid argument for the docker command!"
    EXIT /B 1
  )
)
...
IF "%SCRIPT_CMD%"=="docker" (
  IF "%1" == "package" goto :set_docker_cmd
  IF "%1" == "start" goto :set_docker_cmd
)
...
IF "%1"=="-i" goto set_docker_image
IF "%1"=="-image" goto set_docker_image
...
set "arg=%~2"
IF "%arg%"=="" (
set SCRIPT_ERROR=Appkit profile is required!
goto invalid_cmd_line
)
set firstChar=%arg:~0,1%
IF "%firstChar%"=="-" (
set SCRIPT_ERROR=Expected profile but found %2 instead!
goto invalid_cmd_line
)
set APPKIT_PROFILE=%~2
set "PASS_TO_RUN_EXAMPLE=-p %~2 !PASS_TO_RUN_EXAMPLE!"
SHIFT
SHIFT
goto parse_args
...
IF "%SCRIPT_CMD%" == "docker" (
  IF "!DOCKER_COMMAND!" == "package" (
    CALL :build_docker_image !DOCKER_IMAGE!
    EXIT /B 0
  ) ELSE IF "!DOCKER_COMMAND!" == "start" (
    CALL :build_docker_image_and_run_container !DOCKER_IMAGE!
    EXIT /B 0
  )
)
...
:print_usage
  ...
  IF %CMD%==start (
    echo.
    echo Usage: app-studio.bat %CMD% [-f] [-p port] [-P profile] [-m memory] [-t timeout] [-V]
    echo.
  ...
  IF %CMD%==package (
    echo.
    echo Usage: app-studio.bat %CMD% [-P profile]
    echo.
...
:stop_twigkit
  ...
  IF NOT [%3] == [] (
	...
	echo App Studio stop command executed successfully. If App Studio still appears to be running it may be necessary to manually kill the Java process.
    @REM (sleep 5) &
    @REM spinner $!
...
:build_twigkit
  IF [%APPKIT_PROFILE%] == [] (
    ...
    echo Packaging App Studio using Appkit profile: %APPKIT_PROFILE%
    CALL "%MVNCMD%" clean package -Dtwigkit.profile=%APPKIT_PROFILE% --settings "%SETTINGS%"
  )
...
:build_docker_image
  ...
  IF [%APPKIT_PROFILE%] == [] (
    ...
    CALL docker build -t "%1:latest" .
  ) ELSE (
    echo "Building Docker image of App Studio Enterprise - tagging as %1-%APPKIT_PROFILE%:latest"
    CALL docker build -t "%1-%APPKIT_PROFILE%:latest" --build-arg JAR_PATH=dist/%APPKIT_PROFILE% .
  )

EXIT /B 0

@REM build a docker image and run a container
:build_docker_image_and_run_container
  CALL :build_docker_image %1

  IF [%APPKIT_PROFILE%] == [] (
    echo Running a %1 container on port 8080"
    CALL docker run -p 8080:8080 %1
  ) ELSE (
...
:launch_twigkit
  ...
  IF %FG%==1 (
	...
	@echo on
	IF [%APPKIT_PROFILE%] == [] (
      CALL %MVNCMD% clean jetty:run -Pdevelopment --settings %SETTINGS% %TWIGKIT_OPTS% -Dtwigkit.http.port=%TWIGKIT_PORT% -Dtwigkit.http.stop.port=%STOP_PORT% -Dtwigkit.conf.watch=false
    ) ELSE (
      echo Using Appkit profile: %APPKIT_PROFILE%
      CALL %MVNCMD% clean jetty:run -Dtwigkit.dev.profile=%APPKIT_PROFILE% --settings %SETTINGS% %TWIGKIT_OPTS% -Dtwigkit.http.port=%TWIGKIT_PORT% -Dtwigkit.http.stop.port=%STOP_PORT% -Dtwigkit.conf.watch=false
    )
	@REM removed live config reload (-Dtwigkit.conf.watch=true) due to bug when used on Windows
  ) ELSE (
    @REM run in the background
    IF [%APPKIT_PROFILE%] == [] (
      start /b cmd /c CALL %MVNCMD% clean jetty:run -Pdevelopment --settings %SETTINGS% %TWIGKIT_OPTS% -Dtwigkit.http.port=%TWIGKIT_PORT% -Dtwigkit.http.stop.port=%STOP_PORT% -Dtwigkit.conf.watch=false %SOLR_OPTS%>"%TWIGKIT_LOGS_DIR%/app-studio-%TWIGKIT_PORT%-console.log" 2>&1
    ) ELSE (
      echo Running App Studio using Appkit profile: %APPKIT_PROFILE%
      start /b cmd /c CALL %MVNCMD% clean jetty:run -Dtwigkit.dev.profile=%APPKIT_PROFILE% --settings %SETTINGS% %TWIGKIT_OPTS% -Dtwigkit.http.port=%TWIGKIT_PORT% -Dtwigkit.http.stop.port=%STOP_PORT% -Dtwigkit.conf.watch=false %SOLR_OPTS%>"%TWIGKIT_LOGS_DIR%/app-studio-%TWIGKIT_PORT%-console.log" 2>&1
    )
	@REM removed live config reload (-Dtwigkit.conf.watch=true) due to bug when used on Windows
